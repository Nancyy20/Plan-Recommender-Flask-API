...
def MF_ALS_Rest(train, test):
    ...
    for rank in ranks:
        ...
        model = als.fit(train)
        predictions = model.transform(train)  ##########  here ive changed test to train
        # print('predicts counts-------',predictions.count())
...
def recommendationsRest(model, userID, rest, city):
    ...
    recsforArr = nrecommendations.join(rest, on='restID').select('userID', 'restID', 'name', 'ratings', 'longitude',
                                                                 'latitude', 'city')
...
def recommendationsRestPlan(model, userID, rest, city, times):
    ...
    recommendations = nrecommendations.join(rest, on='restID').select('userID', 'restID', 'name', 'ratings',
                                                                      'longitude', 'latitude', 'city')
    # print("recommendationsss------------------")
    # print(recommendations.show())
    joinedtoTimes = recommendations.join(selectTime, on='restID', how='inner')
    print("joinedtoTimes------------------")
    print(joinedtoTimes.show())
    # filtered = joinedtoTimes.filter((joinedtoTimes.userID == userID) & (joinedtoTimes.city == city))  #changed this to filter only by city
    filtered = joinedtoTimes.filter((joinedtoTimes.city == city))
    print("filtered in recommendationsRestPlan------------- ")
    filtered.show()
